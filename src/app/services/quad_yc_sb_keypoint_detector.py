...
class QuadYCShortBottomKeypointDetectorService:
    ...
    def _find_max_area_quad_with_short_bottom(self, mask: np.ndarray, depth_image: np.ndarray, ntile_divisor: float = 4.0) -> Optional[np.ndarray]:
        ...
        # Refine bottom corners for better bottom grasp points
        if best_quad is not None:
            # Sort points: Top-Left, Top-Right, Bottom-Left, Bottom-Right
            sorted_y = best_quad[np.argsort(best_quad[:, 1])]
            top_pts = sorted_y[:2]
            bot_pts = sorted_y[2:]
            ...
            new_bl, new_br = bl, br
            max_dist_bl, max_dist_br = float('-inf'), float('-inf')

            # Search through the contour points to find a Shorter Bottom Base quadrilateral for bottom corners (graspe points)
            for ctr in contours[0]:
                pt = ctr[0]
                if pt[1] < min_y:
                    continue
                
                if bl[0] < pt[0] < approx_q1_bx:
                    d = np.linalg.norm(pt - bl)
                    if max_dist_bl < d:
                        max_dist_bl = d
                        new_bl = pt
                elif approx_q3_bx < pt[0] < br[0]:
                    d = np.linalg.norm(pt - br)
                    if max_dist_br < d:
                        max_dist_br = d
                        new_br = pt
            
            # Ensure new points have valid depth
            new_br = _find_nearest_nonzero_depth((new_br[0], new_br[1]), depth_image, mask)
            new_bl = _find_nearest_nonzero_depth((new_bl[0], new_bl[1]), depth_image, mask)

            best_quad = np.array([tl, tr, new_br, new_bl], dtype=np.int32)
